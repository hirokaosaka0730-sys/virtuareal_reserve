...
function loadReservations() {
  try { return (JSON.parse(localStorage.getItem(storageKey)) || []).map((record) => ({ ...record, slot: reservationSlots.includes(record.slot) ? record.slot : "予約1" })); } catch { return []; }
}
function saveReservations() { localStorage.setItem(storageKey, JSON.stringify(reservations)); }
...
function render() {
  const { year, month } = monthDate(); const days = new Date(year, month, 0).getDate();
  ...
  const grouped = Object.groupBy(reservations.filter((r) => r.date.startsWith(`${year}-${String(month).padStart(2, "0")}`)), (r) => `${r.date}:${r.slot}`);
  let html = "<thead><tr><th>予約 / 日付</th>";
  for (let day=1; day<=days; day++) { const date = new Date(year, month - 1, day); const weekend = [0,6].includes(date.getDay()); html += `<th class="${weekend ? "weekend-head" : ""}"><span class="date-number">${day}</span>${["日","月","火","水","木","金","土"][date.getDay()]}</th>`; }
  html += "</tr></thead><tbody>";
...
function persistFromForm() { const data = Object.fromEntries(new FormData($("#reservationForm"))); if (!data.date || !data.customer) return; if (editingId) reservations = reservations.map((r) => r.id === editingId ? { ...r, ...data } : r); else reservations.push({ id: crypto.randomUUID(), ...data }); saveReservations(); render(); }

$("#monthPicker").value = isoToday(); render();
$("#parseReservation").addEventListener("click", () => {
  ...
  $("#parseHint").textContent = `${parsed.customer || "予約"}様を${Number(parsed.date.slice(5, 7))}月${Number(parsed.date.slice(8, 10))}日・${parsed.slot}へ登録しました。`;
  render();
});
$("#reservationForm").addEventListener("submit", (event) => { if (event.submitter?.value === "cancel") return; event.preventDefault(); persistFromForm(); $("#reservationDialog").close(); });
...
$("#deleteReservation").addEventListener("click", () => { reservations = reservations.filter((r) => r.id !== editingId); saveReservations(); render(); $("#reservationDialog").close(); });
...
$("#restoreJson").addEventListener("change", async (event) => { const file = event.target.files[0]; if (!file) return; try { const imported = JSON.parse(await file.text()); if (!Array.isArray(imported)) throw new Error(); reservations = imported; saveReservations(); render(); } catch { alert("予約データのJSONファイルを選択してください。"); } event.target.value = ""; });
